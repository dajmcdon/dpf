...
theta = c(363.475,125.05,-11.81757,-34.91605,366.5073,
              0.519518,0.212499,0.7901238,0.0989846,0.7815691,
              0.2194727,0.03586685,0.1342895,0.2435351)
...
library(ggplot2)
ss = factor(convert10to4(bestpath), labels = 
              c('constant tempo', 'decelerating','accelerating','stress'))
plotdf = data.frame(measure = tempos$note_onset, tempo = c(y), 
                  inferred = c(kal$ests), state = ss)
ggplot(plotdf) + 
  ...
  scale_color_brewer(palette='Spectral') + theme_minimal(base_family = 'Times') +
