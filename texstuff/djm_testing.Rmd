...
source('../../code/Beam_Search.R') ## Yupeng's original beam search on some music data
```

This plots our discrete states and his.
```{r discrete-states}
lt = ioi
out = yupengMats(lt, sd_l^2, c(mean(y),-40,-40), c(30,20,10)^2, c(.8,.1,.8,.4))
...
source('../dpf/R/stateConversion.R')
nn = length(y)
...
plot(1:nn,filt$preds,col=convert8to4(bestpath),
...
I printed all of it, but it's clear that the predictions **should** be changing.
Repeating the same thing and staying in the first state (as the best path suggests)...
```{r a-few-more-iterates}
(z1 = kf11(out, y, 1,1)) # the first 1 uses y[1], 
    # the second is the state to trans to (base 1 index)
