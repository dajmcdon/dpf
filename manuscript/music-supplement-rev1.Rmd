...
fivecolors = viridis(7, option='plasma',begin=.2)[c(1,5,2,6,3,7,4)] #brewer.pal(5,'Set1')[c(2,1,3,5,4)]
library(dendextend)
```
...
hc_parm = sweep(pvec_ml, 2, apply(pvec_ml,2,median)) %>% as.matrix
SigInvHalf = chol(solve(crossprod(hc_parm)/nrow(pvec_ml)))
hc_parm = dist(hc_parm %*% SigInvHalf) %>% as.matrix
...
if(colorthem) sdends = sdends %>% 
  set('labels_col', value=fivecolors, k=nclusts) %>%
  set('branches_lty', 1) %>%
  set('branches_k_color', value=fivecolors, k=nclusts)
heatmap.2(sDmat,
...
```{r clust-3, fig.height=3, fig.width=5,include=TRUE, fig.cap="Performances in the third cluster.", eval=FALSE}
...
```{r clust-4, eval=FALSE, fig.height=6, fig.width=5,include=TRUE, fig.cap="Performances in the fourth cluster."}
...
The remaining performances are relatively different from all other performances (\autoref{fig:clust-other}). 

```{r clust-other, fig.height=7, fig.width=5,include=TRUE, fig.cap="Performances in the ``other'' cluster.",eval=FALSE}
...
similar = c('Olejniczac 1990','Shebanova 2002','Richter 1976','Milkina 1970')
...
kable(pvec_ml[c(
  'Olejniczac 1990','Shebanova 2002','Richter 1976','Milkina 1970',
