...
\usepackage{pgf}
\usepackage{tikz}
\usetikzlibrary{fit,arrows,automata,shapes}					% fitting shapes to coordinates
...
\title{Markov-Switching State Space Models for Uncovering Musical
  Interpretation}
\runtitle{Switching Models for Musical
  Interpretation}
\author[A]{%
  \fnms{Daniel J.} \snm{McDonald}\corref{}\ead[label=e1]{daniel@stat.ubc.ca}},
\author[B]{%
  \fnms{Michael} \snm{McBride}\ead[label=e2,mark]{michmcbr@iu.edu}},
\author[C]{%
  \fnms{Yupeng} \snm{Gu}\ead[label=e3,mark]{yupeng.gu@gmail.com}},
\and
\author[C]{\fnms{Christopher} \snm{Raphael}\ead[label=e4,mark]{craphael@indiana.edu}}

\runauthor{McDonald, McBride, Gu, and Raphael}
\address[A]{Department of
  Statistics, University of British Columbia, \printead{e1}}
\address[B]{Department of
  Statistics, Indiana University, \printead{e2}}
\address[C]{School of Informatics,
  Computing, and Engineering, Indiana University, \printead{e3,e4}}
...
\begin{abstract}
  ...
  beyond the vague instructions inscribed on the printed page.
  % While music teachers use
  ...
  recordings of Chopin's Mazurka Op.\ 68 No.\ 3 by consummate
  artists---with the goal of 
  elucidating musically interpretable performance decisions. We
  focus specifically on each performer's use musical tempo by
  examining the
  inter-onset intervals of the note attacks in the recording. To
  explain these tempo decisions, we
  develop a switching state space model and estimate it by maximum
  likelihood combined with
  prior information gained from music theory and performance
  practice. We use the estimated
  parameters to quantitatively describe individual performance
  decisions and compare recordings. These comparisons suggest methods
  for informing music 
...
\begin{keyword}
  \kwd{classification and clustering}
  \kwd{Kalman filter}
...
% that can be used as a learning set or as the
% basis for case-based reasoning.
...
representing expressive performances.% found in this domain.
...
% In this regard these is a close
% connection between our work and that of
...
Although the Kalman filter %\autoref{alg:kalman}
...
\begin{figure}
  \centering
  \begin{tikzpicture}[>=latex,text height=1ex,text depth=0.25ex,ampersand replacement=\&]
...
% $i=j$.
% We then standardize each individual distance matrix to have a
...
surrounding A sections. These two recordings are nonetheless grouped
together by the tempo vectors. Our method on the other hand, puts
these four recordings in different groups. The estimated parameters for these four
...
\begin{table}[tb]
  ...
  \caption{The estimated parameters for the four similar performances
    in group four and those for all four by Arthur Rubinstein.
...
%  \sdescription{Some text}
  ...
  \stitle{Additional figures, source code, and musical examples}
  \slink[url]{https://github.com/dajmcdon/dpf}
  \sdescription{The supplementary material online contains the three components.
    First, the R-package ``\texttt{dpf}'' 
    containing code to perform the
    methods described in the article. The package also contains all data
    sets used as examples in the article. We also provide
    additional \texttt{R} code necessary to reproduce all
    analyses and graphics. Second, a separate appendix with additional
    graphics for all clusters and analysis of all 46 recordings. Third,
    MIDI files for a real and synthesized performance by Richter.}
