...
\begin{abstract}
  ...
  inter-onset intervals of the note attacks in the recordings, we
  learn a switching state space model enriched with prior information
  gained from music theory to discover relevant features and perform
  hierarchical 
...
% One issue in our model that the effect of states two and three can
% be flipped. If $\tau_t$ is small enough, and $\sigma_4^2$ and
% $\sigma_2^2$ are large enough, it would be possible to get a
% negative number for the acceleration term when in state 2 or a
% positive number when in state three and still get a reasonably high
% likelihood. This could be fixed by adding a constraint on the ratio
% of the $\tau_t$ and $\sigma$'s 2 and 4.
...
%  \sdescription{Some text}
  ...
  \stitle{Additional figures and source code}
  ...
  \sdescription{The supplementary material online contains the three components.
    ...
    sets used as examples in the article. Second, we provide
    additional \texttt{R} code necessary to reproduce all
    analyses and graphics. Third, a separate appendix with additional
    graphics for all clusters and analysis of all 46 recordings.}
