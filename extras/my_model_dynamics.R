...
logprior <- function(theta,yt){
  ...
  p3 = dbeta(theta[12],2,8,  log=TRUE)
  p4 = dbeta(theta[13],2,8, log=TRUE)
...
rprior <- function(n,yt){
  ...
  sig1 = rgamma(n, shape=10, scale=1)
  sig2 = rgamma(n, shape=3, scale=1)
  sig3 = rgamma(n, shape=3, scale=1)
  ...
  p31 = rbeta(n,2,8)
  p41 = rbeta(n,2,8)
  cbind(muc, sig2eps, mu1, mu2, mu3, sig0, sig1, sig2, 
        p21, p23, p24, p31, p41)
}
...
toOptimize <- function(theta, yt, lt, Npart, badvals=Inf){
  ...
  pmats = musicModeldynamics(lt, theta[1], theta[2], theta[3:5], theta[6:8], theta[9:13], 
                             c(18,0,0), 
                             c(3,1,1))
...
optimizer <- function(perf, lt, Npart=500, ntries = 20, badvals=1e8){
